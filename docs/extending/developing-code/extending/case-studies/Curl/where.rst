...
To find where to put ``Curl`` in Mathics, we look this up the  `WMA docs <https://reference.wolfram.com/language/ref/Curl.html>`_  and then looking at the Related Guides dropdown menu we see:
...
However if we type "Operations" we see "Operations on Vectors" is already there. If we go into the documentation for that section,  we find there is a subection of that called "Mathematical Operations".
